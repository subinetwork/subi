...
PACKAGE_VERSION=$($SRC_PATH/src/subid --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/subid $DEB_BIN
cp $SRC_PATH/src/subi-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/subi-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/subid.1 $DEB_MAN
cp $SRC_DOC/man/subi-cli.1 $DEB_MAN
cp $SRC_DOC/man/subi-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/subid.bash-completion $DEB_CMP/subid
cp $SRC_PATH/contrib/subi-cli.bash-completion $DEB_CMP/subi-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/subid.1
gzip --best -n $DEB_MAN/subi-cli.1
gzip --best -n $DEB_MAN/subi-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/subid $DEB_BIN/subi-cli
