...
DOWNLOAD_URL="https://github.com/subinetwork/subi/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with SUBI_DISABLE_SOMETHING=1
SUBI_DISABLE_WGET="${SUBI_DISABLE_WGET:-}"
#SUBI_DISABLE_IPFS="${SUBI_DISABLE_IPFS:-}"
SUBI_DISABLE_CURL="${SUBI_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$SUBI_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$SUBI_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$SUBI_DISABLE_CURL" ]; then
        return 1
    fi
