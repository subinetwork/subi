...
# subi_hash
RUN git clone https://github.com/subinetwork/subi_hash
RUN cd subi_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} subi
RUN useradd -u ${USER_ID} -g subi -s /bin/bash -m -d /subi subi
...
RUN mkdir /subi-src && \
  ...
  chown $USER_ID:$GROUP_ID /subi-src && \
